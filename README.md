...
测试过程自动录制高清视频（1920x1080），文件命名格式：`test_xx_YYYYMMDD.webm`

| 测试文件 | 视频文件 |
|----------|----------|
| test_01_provider.py | test_01_YYYYMMDD.webm |
| test_02_operator.py | test_02_YYYYMMDD.webm |
| ... | ... |

视频保存于 `videos/` 目录，可用于测试结果复盘和问题排查。

## 订单追踪

`latest_order.txt` 文件记录最近一次测试的订单编号，供后续测试流程使用。

## 注意事项

1. **执行顺序**：测试依赖执行顺序，请确保按 01 → 02 → 03 → 04 → 05 → 06 的顺序运行
2. **视频录制**：每次运行自动录制 HD 视频（1920x1080），存于 `videos/` 目录
